...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        weights = tf.Variable(np.zeros((self.config.n_features,
                                        self.config.n_classes)),
                                        dtype=tf.float32,
                                        name='weights')
        biases = tf.Variable(np.zeros((1, self.config.n_classes)),
                             dtype=tf.float32,
                             name='biases')
        return softmax(tf.matmul(self.input_placeholder, weights) + biases)
