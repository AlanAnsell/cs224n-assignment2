...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        self.input_placeholder = tf.placeholder(shape=(None, self.config.n_features),
                                                dtype=tf.int32)
        self.labels_placeholder = tf.placeholder(shape=(None, self.config.n_classes),
                                                 dtype=tf.float32)
    ...
    def add_embedding(self):
        ...
        batch_embeddings = tf.nn.embedding_lookup(all_embeddings, self.input_placeholder)
        print(batch_embeddings)
        return tf.reshape(batch_embeddings,
    ...
    def add_prediction_op(self):
        ...
        initialize = xavier_weight_init()
        W = tf.Variable(initialize((self.config.n_features * self.config.embed_size,
                                    self.config.hidden_size)))
        b1 = tf.Variable(tf.zeros((self.config.hidden_size,)))
        h = tf.nn.relu(tf.matmul(x, W) + b1)
        h_dropout = tf.nn.dropout(h, 1.0 - self.dropout_placeholder)
        
        U = tf.Variable(initialize((self.config.hidden_size, self.config.n_classes)))
        b2 = tf.Variable(tf.zeros((self.config.n_classes,)))
        return tf.matmul(h_dropout, U) + b2
    ...
    def add_training_op(self, loss):
        ...
        """
        return tf.train.AdamOptimizer(self.config.lr).minimize(self.loss)
