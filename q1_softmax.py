...
def softmax(x):
    ...
    row_maxs = tf.expand_dims(tf.reduce_max(x, axis=1), axis=1)
    expd = tf.exp(x - row_maxs)
    row_sums = tf.expand_dims(tf.reduce_sum(expd, axis=1), axis=1)
    out = expd / row_sums
...
def cross_entropy_loss(y, yhat):
    ...
    return -tf.reduce_sum(tf.log(yhat) * tf.to_float(y))
